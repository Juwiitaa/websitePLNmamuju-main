...
@section('content')
    ...
    <div class="absolute hidden shadow-xl justify-center w-10/12 sm:w-1/2 lg:w-[35%] lg:mt-15 xl:mt-0  h-fit items-center px-5 pb-5 pt-2 rounded-lg  right-1/2 translate-x-1/2 top-1/2 -translate-y-1/2 backdrop-blur-lg  bg-white"
        id="Kunjungan">
        <button type="button" id="logOutPetugas" class="flex items-center bg-primary hover:bg-red-600 cursor-pointer transition-all duration-200 w-fit p-1.5 rounded-lg border-0 outline-none">
            <i class='bx bx-log-out text-3xl text-white'></i><span class="text-white ml-1">LogOut</span>
        </button>
    ...
    <script>
            ...
            try {
                await $.ajax({
                    ...
                    success: async function(res) {
                        ...
                        wadahToken.innerHTML = "<span class='text-black font-semibold'>" + res.token + "</span>";
                        await Swal.fire({
                            title: "Berhasil!",
                ...
                let message = err.responseJSON ? err.responseJSON.message : 'Terjadi kesalahan sistem';
                await Swal.fire({
                    title: "Gagal!",
                    text: message,
                    icon: 'error',
        ...
        const btnLogOutPetugas = document.getElementById('logOutPetugas');
        if (btnLogOutPetugas) {
            btnLogOutPetugas.style.cursor = 'pointer';
                ...
                try {
                    await $.ajax({
                        ...
                        success: async function(res) {
                            await Swal.fire({
                                title: "Berhasil!",
                                text: "Identitas petugas berhasil dihapus dari perangkat.",
                                icon: 'success',
                                confirmButtonText: "OK"
                            });
                        ...
                        error: async function() {
                            await Swal.fire({
                                title: "Gagal!",
                                text: "Gagal memproses LogOut pada server.",
                                icon: 'error',
                                confirmButtonText: "OK"
                            });
                        }
